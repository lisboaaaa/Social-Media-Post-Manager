...
const { data: linkData, error: linkErr } = await admin.auth.admin.generateLink({ type: "magiclink", email: "laura.lisboa@daredata.engineering" });
if (linkErr) throw linkErr;
const anon = createClient(SUPABASE_URL, ANON_KEY);
const { data: verifyData, error: verifyErr } = await anon.auth.verifyOtp({ type: "magiclink", token_hash: linkData.properties.hashed_token });
...
await page.screenshot({ path: "scripts/screenshot-calendar.png", fullPage: true });
